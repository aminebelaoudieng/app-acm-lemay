...
<div class="list-page">
    ...
    <table class="bg-grey" cellpadding="0" cellspacing="0">
        ...
        <tr>
            ...
            @foreach($ficheMaster->fichesVendu()->limit(4)->get() as $fiche)
            <td class="info-fiche">@if(($fiche->type_copropriete=="divise")) {{ money($fiche->comparable_vendu_prix_evaluation) }} @endif</td>
            @endforeach
        </tr>
        @endif
        <tr>
        ...
        <tr>
            ...
            <td class="info-fiche">-</td>
            @foreach($ficheMaster->fichesVendu()->limit(4)->get() as $fiche)
            <td class="info-fiche">{{ $fiche->comparable_vendu_delais_vente}} {{ __('pdf.days') }} </td>
            @endforeach
        </tr>
        <tr>
    ...
    <sup class="txt-center" style="margin-top:-20px;">{{ __('pdf.price_sqft_note') }}</sup>
    <table>
        <tr>
            <td>
            ...
            <td>
                ...
                <p class="probablite">{{ __('pdf.probable_sale_price_range_sqft', ['min' => money($ficheMaster->moyenneRatioPrixHabitableVendu*0.95), 'max' => money($ficheMaster->moyenneRatioPrixHabitableVendu*1.05)]) }}</p>

                @else
                <p class="table-title" style="font-size:12px;">{{ __('pdf.probable_listing_price_requested') }}</p>
                <p class="probablite">{{ __('pdf.probable_sale_price_range_requested', ['min' => money($ficheMaster->moyennePrixVente*0.95), 'max' => money($ficheMaster->moyennePrixVente*1.05)]) }}</p>
                @endif
            </td>
        </tr>
    </table>
