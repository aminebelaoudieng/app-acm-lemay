...
<div class="list-page">
    ...
    <table class="bg-grey" cellpadding="0" cellspacing="0">
        ...
        <tr>
            <td class="main-label">{{ __('pdf.gross_income') }}</td>
            <td class="info-fiche">{{ money($ficheMaster->rendement_revenus_brut) }}</td>
            @foreach($ficheMaster->fichesVendu()->limit(4)->get() as $fiche)
            <td class="info-fiche">{{ money($fiche->rendement_revenus_brut) }} </td>
            @endforeach
        </tr>
        <tr>
            <td class="main-label">{{ __('pdf.expenses') }}</td>
            <td class="info-fiche">{{ money($ficheMaster->rendement_depense) }}</td>
            @foreach($ficheMaster->fichesVendu()->limit(4)->get() as $fiche)
            <td class="info-fiche">{{ money($fiche->rendement_depense) }} </td>
            @endforeach
        </tr>
        <tr>
            <td class="main-label">{{ __('pdf.net_income') }}</td>
            <td class="info-fiche">{{ money($ficheMaster->rendement_revenus_brut-$ficheMaster->rendement_depense) }}</td>
            @foreach($ficheMaster->fichesVendu()->limit(4)->get() as $fiche)
            <td class="info-fiche">{{ money($fiche->rendement_revenus_brut-$fiche->rendement_depense) }} </td>
            @endforeach
        </tr>
    </table>
    ...
    <table>
        <tr>
            ...
            <td>
                ...
                <p class="probablite">{{ __('pdf.probable_sale_price_7_of_10', ['min' => money($ficheMaster->moyennePrixVente*0.95), 'max' => money($ficheMaster->moyennePrixVente*1.05)]) }}</p>
                @endif
            </td>
        </tr>
    </table>
