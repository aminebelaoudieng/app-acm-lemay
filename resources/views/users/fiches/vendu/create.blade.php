...
<div class="row mb-5">
    <div class="col-lg-12 margin-tb">
        <div class="d-flex">
            <a class="btn btn-primary mr-3" href="{{ route('fiches.edit',$ficheMaster->id).'#vendu' }}">{{ __('profile.back') }}</a>
...
<div class="tab-content p-5" id="mainContent">
    ...
    <div class="tab-pane fade show active" id="general" role="tabpanel" aria-labelledby="general-tab">
        {!! Form::model($ficheMaster, ['method' => 'POST', 'files' => true, 'route' => ['fiches.vendu.store',$ficheMaster->id],'class'=>'row']) !!}
        ...
        <div class="col-xs-12 col-sm-12 col-md-2">
            <div class="nav flex-column nav-pills" id="v-pills-tab" role="tablist" aria-orientation="vertical">
                ...
                <a class="nav-link" id="v-pills-caracteristiques-tab" data-toggle="pill" href="#v-pills-caracteristiques" role="tab" aria-controls="v-pills-caracteristiques" aria-selected="false">{{ __('fiches_subtabs.characteristics') }}</a>
            </div>
        </div>
        <div class="col-xs-12 col-sm-12 col-md-10">
            <div class="tab-content" id="v-pills-tabContent">
                @include('share.errors')
                <h2>{{ __('fiches_form.new_sold_property') }}</h2>
                <div class="tab-pane fade show active" id="v-pills-adresse" role="tabpanel" aria-labelledby="v-pills-adresse-tab">
                    @include('users.fiches.sub-tabs.adresse',['fiche'=>null])
                </div>
                <div class="tab-pane fade" id="v-pills-prixdate" role="tabpanel" aria-labelledby="v-pills-prixdate-tab">
                    @include('users.fiches.sub-tabs.prix-date-vendu',['fiche'=>null])
                </div>
                <div class="tab-pane fade" id="v-pills-caracteristiques" role="tabpanel" aria-labelledby="v-pills-caracteristiques-tab">
                    @include('users.fiches.sub-tabs.caracteristiques',['fiche'=>null])
                </div>
            </div>
        </div>
        {!! Form::close() !!}
