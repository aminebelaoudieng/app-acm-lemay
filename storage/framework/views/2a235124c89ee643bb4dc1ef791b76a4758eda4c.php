...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        if (!csrfToken) {
            alert('<?php echo e(__("profile.csrf_error")); ?>');
            return;
        }
        ...
        setTimeout(function() {
            resize = $('#upload-demo').croppie({
                ...
                url: "<?php echo e($photo); ?>?v=" + Date.now(),
                boundary: {
                    width: 200,
                    height: 200
                }
        ...
        $('#image_file').on('change', function() {
            ...
            if (!allowedTypes.includes(file.type)) {
                $('#error-message').text('<?php echo e(__("profile.unsupported_file_format")); ?>');
                $('#upload-error').show();
                return;
            }
            ...
            if (file.size > 2 * 1024 * 1024) {
                $('#error-message').text('<?php echo e(__("profile.file_too_large")); ?>');
                $('#upload-error').show();
                return;
            }
            ...
            reader.onload = function(e) {
                if (resize) {
                    resize.croppie('bind', {
                        ...
                        $('#button-text').text('<?php echo e(__("profile.change_photo")); ?>');
                        ...
                        if (window.location.href.includes('/create')) {
                            ...
                            setTimeout(function() {
                                resize.croppie('result', {
                                    ...
                                    $('#success-message').text('<?php echo e(__("profile.image_upload_success")); ?>');
        ...
        $('.upload-image').on('click', function(ev) {
            ...
            $(this).prop('disabled', true).html('<i class="fas fa-spinner fa-spin mr-2"></i><?php echo e(__("profile.saving")); ?>...');
            ...
            resize.croppie('result', {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        if(data.file) {
                            $('#success-message').text('<?php echo e(__("profile.photo_updated_success")); ?>');
                            $("#upload-success").fadeIn();
                            $("input[name='photo']").val(data.file);
                        } else {
                            $('#error-message').text('<?php echo e(__("profile.photo_save_error")); ?>');
                            $('#upload-error').fadeIn();
                        }
                        
                        $('.upload-image').prop('disabled', false).html('<i class="fas fa-save mr-2"></i><?php echo e(__("profile.update_photo")); ?>');
                    },
                    error: function(xhr, status, error) {
                        console.log('AJAX Error:', xhr.status, xhr.responseText);
                        
                        var errorMessage = '<?php echo e(__("profile.upload_technical_error")); ?>';
                        if (xhr.status === 419) {
                            errorMessage = '<?php echo e(__("profile.session_expired")); ?>';
                        } else if (xhr.responseJSON && xhr.responseJSON.message) {
                            errorMessage = xhr.responseJSON.message;
                        } else if (xhr.status === 413) {
                            errorMessage = '<?php echo e(__("profile.file_too_large_server")); ?>';
                        } else if (xhr.status === 422) {
                            errorMessage = '<?php echo e(__("profile.invalid_file_format")); ?>';
                        }
...
<div class="col-xs-12 col-sm-12 col-md-12">
    <div class="form-group">
        <strong><?php echo e(__('profile.profile_photo')); ?></strong>
        ...
        <div class="row">
            ...
            <div class="col-md-7">
                <button type="button" class="btn btn-outline-primary btn-lg btn-block d-flex align-items-center justify-content-center" id="choose_image_file">
                    ...
                    <span id="button-text"><?php echo e(__('profile.choose_photo')); ?></span>
                </button>
                <input type="file" id="image_file" class="d-none" accept="image/*">
                
                <?php if(Auth::user()->is_admin): ?>
                <input type="hidden" id="user_id" name="user_id" value="<?php echo e((isset($user)) ? $user->id : $nextId); ?>">
                <?php endif; ?>
                <?php if($photo && $photo!=""): ?>
                <input type="hidden" name="photo" value="<?php echo e(explode('/',$photo)[5]); ?>">
                <?php endif; ?>
                
                <button class="btn btn-success btn-block upload-image <?php if($photo==''): ?> d-none <?php endif; ?> mt-3" style="margin-top:2%; <?php if(request()->is('*/create')): ?> display: none !important; <?php endif; ?>">
                    <i class="fas fa-save mr-2"></i><?php echo e(__('profile.update_photo')); ?>
                ...
                <div class="alert alert-info mt-3" id="upload-info" style="display: none;">
                    <i class="fas fa-info-circle mr-2"></i><?php echo e(__('profile.photo_formats_info')); ?>

                </div>
            </div>
        </div>
    </div>
</div><?php /**PATH /home/lemaycon/app.lemayconsulting.com/resources/views/profile/plugins/photo.blade.php ENDPATH**/ ?>